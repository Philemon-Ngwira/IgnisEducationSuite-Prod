...
namespace SchedulingTester.TimeTableGenerator
{
    ...
    public class RealisticTimetableGenerator
    {
                ...
                for (int i = 0; i < daySlots.Count; i++)
                {
                    ...
                    if (prepActivity != null && slot.StartTime >= prepActivity.StartFrom)
                    {
                        ...
                        result.Add(new TimeSlot
                        {
                            ...
                            SubjectName = prepActivity.Name
                        });

                        if (debug) Console.WriteLine($"Slot {i}: {slot.StartTime}-{slot.EndTime} -> Prep Activity ({prepActivity.Name})");
                        continue;
                    }
